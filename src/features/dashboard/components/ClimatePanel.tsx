...
const ClimatePanel: React.FC<{ rows: Row[] }> = ({ rows }) => {
  ...
  const option = useMemo<EChartsOption | null>(() => {
    ...
    const weekPrefix = lang === 'es' ? 'S' : 'W';
    const casesLabel = lang === 'es' ? 'Casos' : 'Cases';
    const rainLabel = lang === 'es' ? 'Lluvia (mm)' : 'Rain (mm)';
    const rainAxisLabel = lang === 'es' ? 'Lluvia' : 'Rain';
    const tempLabel = lang === 'es' ? 'Temp (°C)' : 'Temp (°C)';

    const x = keys.map((k) => `${Math.floor(k / 100)}·${weekPrefix}${String(k % 100).padStart(2, '0')}`);
    ...
    return {
      ...
      legend: { data: [casesLabel, rainLabel, tempLabel], textStyle: { color: AXIS }, top: 0 },
      ...
      yAxis: [
        { type: 'value', name: casesLabel, position: 'left', nameTextStyle: { color: AXIS }, axisLabel: { color: AXIS, fontSize: 10 }, splitLine: { lineStyle: { color: SPLIT } } },
        { type: 'value', name: rainAxisLabel, position: 'right', nameTextStyle: { color: AXIS }, axisLabel: { color: AXIS, fontSize: 10 }, splitLine: { show: false } },
      ...
      series: [
        { name: casesLabel, type: 'line', data: casos, yAxisIndex: 0, smooth: true, showSymbol: false, areaStyle: { color: 'rgba(0,240,255,0.18)' }, lineStyle: { color: '#00f0ff', width: 1.5 }, z: 3 },
        { name: rainLabel, type: 'bar', data: precip, yAxisIndex: 1, itemStyle: { color: 'rgba(59,130,246,0.45)' } },
        { name: tempLabel, type: 'line', data: temp, yAxisIndex: 2, smooth: true, showSymbol: false, lineStyle: { color: '#ff6600', width: 1.5, opacity: 0.85 } },
      ],
      dataZoom: x.length > 60 ? [{ type: 'inside' }, { type: 'slider', height: 16, bottom: 6, textStyle: { color: AXIS } }] : undefined,
    };
  }, [clima, rows, lang]);

  if (!option) return <div className={styles.mapLoading} style={{ height: 360 }}>{lang === 'es' ? 'Cargando clima…' : 'Loading climate data…'}</div>;
